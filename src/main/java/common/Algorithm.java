...
public class Algorithm {
    ...
    public int countLine() {
        int countLine = 0;
        try {
            BufferedReader br = new BufferedReader(new FileReader(new File(path+"/src/input.txt")));
    ...
    public void normalize() {
        ...
        try {
            ...
            int count = 1;
            br = new BufferedReader(new FileReader(new File(path+"/src/input.txt")));
            PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(path+ "/src/output.txt", true)));
            String line;
            while ((line = br.readLine()) != null) {
                if (isLineEmpty(line)) {
                    continue;
